...
def login():
    if request.method == 'POST':
        ...
        if user['rol'] == 'administrador':
            cursor.execute("SELECT id_administrador FROM administrador WHERE id_usuario = %s", [
                           user['id_usuario']])
            ...
            cursor.execute("SELECT id_cajero FROM cajero WHERE id_usuario = %s", [
                           user['id_usuario']])
            ...
            cursor.execute("SELECT id_tutor FROM tutor WHERE id_usuario = %s", [
                           user['id_usuario']])
            ...
            cursor.execute(
                "SELECT * FROM competidor WHERE email = %s", [user['email']])
    ...
    if request.method == 'POST':
        try:
            ...
            query_usuario = """
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            """
            cursor.execute(query_usuario, (nombres, apellidos,
                           email, telefono, contrasena, rol))
...
def registrar_competidor():
    if request.method == 'POST':
        ...
        try:
            ...
            """
            cursor.execute(insert_competidor, (ci, fecha_nacimiento, colegio, curso,
                           departamento, provincia, nombre, apellido, email, telefono, estado, id_tutor))
            ...
            cursor.execute(insert_inscripcion,
                           (id_competidor, id_tutor, fecha_actual))
...
def obtener_tutores():
    try:
        cursor = mysql.connection.cursor()
        cursor.execute(
            "SELECT id_usuario, nombre, apellido FROM Usuario WHERE rol = 'tutor'")
